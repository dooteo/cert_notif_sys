...
require_once ('admin/libraries/timestamps2.php');
...
$my_hash = array('type' => 'sha1', 'hash' => '');
$my_hash['hash'] = hash($my_hash['type'], "content to stamp"); // <-- To Search in TSA Server ;)

echo 'hash type: '. $my_hash['type']. 'myhash: '.$my_hash['hash'].'<br>';

$requestfile_path = tsa_createRequestfile($my_hash);

$response = tsa_signRequestfile(
				$requestfile_path, 
...
echo tsa_getTimestampFromAnswer($response['response_string']); //1299098823
echo '<br><br>---------^ getTimestampFromAnswer-------------<br><br>';
$validate = tsa_validate(
				$my_hash, 
...
$my_hash = array('type' => 'sha1', 'hash' => sha1("im not the right hash"));
$validate = tsa_validate(
				$my_hash, 
				$response['response_string'], 
				$response['response_time'], 
