...
#
#	Purpose: Contains functions used to inject data from MySQL 
#       into an empty PDF form
#
...
class Zdn_pdftk { 
	function __construct(){} // Dooteo's hack
	// ddf = means dump data fields, so this kind of file is plain txt file
	function dump_data_fields ($pdf_file, $ddf_file) {
		$status = exec("/usr/bin/pdftk $pdf_file dump_data_fields > $ddf_file");
		return $status;
	}
	function parse_ddf_file($ddf_file) {
		$fields = "";
		$group_id = 0;
		...
		while (!feof($fd)) {
			...
			} else if (strpos($line, "FieldType: ") !== false) {
				$fields .= trim(strstr($line, " ")) . "||";
			} else if (strpos($line, "FieldName: ") !== false) {
				$fields .= trim(strstr($line, " "));
			}
		}
	...
	function simpleBootLoader( $stack ) {
		require_once 'system/libraries/' . 
		str_replace( '\\', DIRECTORY_SEPARATOR, $stack ) .'.php';
	}
	public function make_zend_pdf ($fdf_data_strings, $fdf_data_names, 
						$pdf_original, $pdf_filename) {
...
#############################################################################
		...
		public function make_pdf($fdf_data_strings, $fdf_data_names, $fields_hidden,
					 $fields_readonly, $pdf_original, $pdf_filename, 
					 $tmp_dir) {
			// Create the fdf file
			$fdf = $this->forge_fdf('', $fdf_data_strings, 
						$fdf_data_names, $fields_hidden, 
						$fields_readonly);
			
			// Save the fdf file temporarily - 
			// make sure the server has write permissions in the 
			// folder you specify in tempnam()
			$fdf_fn = tempnam($tmp_dir, "fdf");
			$fp = fopen($fdf_fn, 'w');
			...
			if($fp) {
				...
				// Actually make the PDF by running pdftk - 
				// make sure the path to pdftk is correct
				passthru("/usr/bin/pdftk $pdf_original fill_form $fdf_fn output  $pdf_filename");
		...
		public function make_downloadable_pdf($fdf_data_strings, $fdf_data_names,
						$fields_hidden, $fields_readonly, 
						$pdf_original, $pdf_filename, $tmp_dir) {
			// Create the fdf file
			$fdf = $this->forge_fdf('', $fdf_data_strings, 
						$fdf_data_names, $fields_hidden, 
						$fields_readonly);
			...
			if($fp) {
				...
				// The PDF will be output directly to the browser - apart from the original PDF file, no actual PDF will be saved on the server.
