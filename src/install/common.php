...
// --------------------------
// Description: search into a directory to find languages files
//
//
// --------------------------

function languages_list ($directory){

	$count = 0;
	$languages_lst = array();
	$dirdesc = opendir($directory);	
	$filename = readdir ($dirdesc);
	
	while ($filename){
		// Languages extension should be '.php'
		if (ereg(".php", $filename)) {
			
			// get only filename, without extension
			$languages_lst[$count++] = basename($filename, ".php");
...
function common_getparameter ($_key) {
